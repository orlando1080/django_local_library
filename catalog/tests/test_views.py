...
class LoanedBookInstanceByUserListViewTest(TestCase):
    def setUp(self):
        # Create two users.
        test_user1 = User.objects.create_user(username='testuser1', password='finishingtouch1')
        test_user2 = User.objects.create_user(username='testuser2', password='finishingtouch7')
    ...
    def test_logged_in_uses_correct_template(self):
        login = self.client.login(username='testuser1', password='finishingtouch1')
        response = self.client.get(reverse('my-borrowed'))
...
class AuthorCreateView(TestCase):
    def setUp(self):
        # Create a user
        test_user1 = User.objects.create_user(username='testuser1', password='1X<ISRUkw+tuK')
        test_user2 = User.objects.create_user(username='testuser2', password='2HJ1vRV0Z&3iD')
        ...
        # Give test_user2 permission to create Author.
        permission = Permission.objects.get(name='set book as returned')
        test_user2.user_permissions.add(permission)
        test_user2.save()

    def test_the_initial_date_of_death_is_correct(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)

        initial_date_of_death = '11/06/2022'
    ...
    def test_logged_in_with_permission_to_create_author(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
    ...
    def test_redirects_to_authors_details_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('author-create'), {'first_name': 'James', 'last_name': 'Gun', 'date_of_birth': '08/03/1985'})
